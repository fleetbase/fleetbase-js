...
Release metadata consistently targets 2.0.0. Per the clarified release policy, merging a reviewed release branch into main automatically tags, publishes to npm, verifies the registry artifact, and creates the GitHub release in `release.yml`. The former enable flag, second environment approval, and shared tag-token dependency are removed. One-time npm trusted publishing must authorize `release.yml` with no environment restriction. No package was published during this work. Main is protected by one approving review plus strict `CI success`/`CodeQL` checks; master remains a compatibility branch. Finish native acceptance before merging, because merging starts publication.
